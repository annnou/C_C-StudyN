...
int Game::Run()
{
	...
	ID3D11Buffer* constantBuffer = nullptr;
	{
		...
		bufferDesc.ByteWidth = sizeof(MatricesPerFrame);
		bufferDesc.Usage = D3D11_USAGE_DEFAULT;
		...
		if (FAILED(hr)) {
			MessageBox(hWnd, L"定数バッファーを作成できませんでした。", L"ERROR", MB_OK);
			return -1;
		}
	}
	...
	while (true) {
		...
		// 定数バッファーを更新
		immediateContext->UpdateSubresource(constantBuffer, 0, NULL, &matricesPerFrame, 0, 0);
		...
		// 頂点シェーダーに定数バッファーを設定
		ID3D11Buffer* constantBuffers[1] = { constantBuffer };
