...
int Game::Run()
{
	...
	// 頂点データの配列
	VertexPositionNormal vertices[divideCount + 1] = {};
	// 上面
	// 中心点
	vertices[0].position = XMFLOAT3(0.0f, height / 2, 0.0f);
	...
	for (size_t triangle = 0; triangle < divideCount; triangle++) {
		...
		// 位置座標
		vertices[triangle + 1].position.x = radius * XMScalarCos(angle);
		vertices[triangle + 1].position.y = height / 2;
		vertices[triangle + 1].position.z = radius * -XMScalarSin(angle);
		// 法線ベクトル
		vertices[triangle + 1].normal = XMFLOAT3(0.0f, 1.0f, 0.0f);
	}
	...
	for (UINT32 triangle = 0; triangle < divideCount; triangle++) {
		...
		indices[3 * triangle + 2] = (triangle + 1) % divideCount + 1;
	}
	...
	inputLayout = InputLayout::Create(graphicsDevice,
		VertexPositionNormal::GetInputElementDescs(), VertexPositionNormal::GetInputElementDescsLength(),
