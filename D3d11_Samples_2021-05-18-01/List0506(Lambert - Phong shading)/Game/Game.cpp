...
int Game::Run()
{
    ...
    for (size_t triangle = 0; triangle < divideCount; triangle++) {
        const auto angle = XM_2PI / divideCount * triangle;
        // vertices配列にはTop面の頂点数の分(divideCount)だけずらしてアクセスする
    ...
    // Bottom
    for (UINT32 triangle = 0; triangle < divideCount - 2; triangle++) {
        const UINT32 indexOffset = 3 * (divideCount - 2);
        indices[indexOffset + 3 * triangle + 0] = 0 + divideCount;
        indices[indexOffset + 3 * triangle + 1] = triangle + 1 + divideCount;
        indices[indexOffset + 3 * triangle + 2] = triangle + 2 + divideCount;
    }
    // Side
