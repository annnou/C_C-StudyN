...
// 
//=============================================================================
#include <d3dcompiler.h>	// シェーダーコンパイラー
...
int Game::Run()
{
	...
	hr = D3DCompileFromFile(
		L"BasicVertexShader.hlsl",	// HLSLシェーダーファイル名
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",						// エントリーポイント
		"vs_5_0",					// コンパイルするシェーダーのターゲット
		D3DCOMPILE_DEBUG, 0,
	...
	*/
	...
	hr = graphicsDevice->CreateInputLayout(
		...
		bytecode->GetBufferPointer(),	// 入力を受け取る頂点シェーダーのバイトコード
		bytecode->GetBufferSize(),		// バイトコードのサイズ
		&inputLayout);
	if (FAILED(hr)) {
		OutputDebugString(L"入力レイアウトを作成できませんでした。");
	}

	// ID3D11VertexShaderとID3D11InputLayoutを作成したらバイトコードは削除しても良い
